...
Generate a JSON object conforming to the [`UpdateProviderPromptRequest`](/api-reference/providers/prompts/update-provider-prompt) payload. Due to HasMCP's patch methodology, you only map the internal top-level keys inside the `prompt` object that demand modification. 
