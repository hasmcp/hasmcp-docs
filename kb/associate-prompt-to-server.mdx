...
If the `promptID` mapping exists and the auth token has execution authorization on the targeted `serverID`, the orchestrator immediately saves the binding and responds with a `200 OK`.
