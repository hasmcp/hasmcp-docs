...
**MCP Server URL:**
`https://app.hasmcp.com/mcp/<SERVER_ID>?token=<YOUR_ACCESS_TOKEN>`

**Authentication:**
Choose `No Auth` since we passed the token as query string already.

Click `Create` button at the bottom of the modal window to activate the MCP Server.
...
- **Token Mismatch**: Ensure there are no spaces or extra characters in the `?token=` value.
- **Endpoint Errors**: Ensure the `SERVER_ID` in the URL matches the one provided in your dashboard.
- **Method Support**: ChatGPT Actions primarily use standard HTTP methods (GET/POST). Ensure your tools are configured correctly in the HasMCP Provider.
