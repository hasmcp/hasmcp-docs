...
- **Name**: `sendEmail`
- **Description**: `Send an email. The body requires a JSON object with a 'raw' field containing a base64url encoded RFC 2822 email message.`
...
<Steps>
    ...
    <Step>Ask your MCP Client: _"Search for the last email from hasmcp.com."_</Step>
    <Step>
    Watch the logs:
    - You will see the request hit `/users/me/messages` with `q=from:hasmcp.com`.
    - You will see the JSON response from Google listing message IDs.
    </Step>
    <Step>
    Ask your MCP Client: _"Send an email to me saying Hello."_
    - You will see the Model attempt to construct the `raw` base64 string.
    </Step>
</Steps>
